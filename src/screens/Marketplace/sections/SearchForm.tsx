...
import { USAState, getState } from '@/utility/usaStateAbbr';
...
const ProxyComponent = () => {
  ...
  const registeredStates: USAState[] = useSelector(selectStatesFilters);

  let states: USAState[] = registeredStates.map((state: any) => ({
  ...
  const handleSearchButtonClick = () => {
    history.push(
      `/marketplace?state=${selectedState}&search-type=${switchValue}`
    );
  };

  const onChangeSearchType = () => {
    switchValue === 'walls' && setSwitchValue('artists');
    switchValue === 'artists' && setSwitchValue('walls');
  };
...
export const SearchForm: React.FC = (props) => <ProxyComponent {...props} />;
